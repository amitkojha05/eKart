...
const Item = ({ id, title, price, desc, rating }) => {
    ...
    }, [id, wishlist]); // Added id and wishlist to the dependency array
    ...
    return (
        <div className="flex flex-col xsm-mx:w-[20%] xsm-mx:min-w-32 xs-mx:w-[20%] sm-mx:w-[25%] sm-mx:min-w-40 md-mx:w-[30%] md-mx:min-w-52 md:min-w-72 md:w-[23%] gap-1.5">
            <div className="bg-gray-100 rounded-lg flex items-center justify-center relative overflow-hidden">
                <Link to={`/product/${id}`}>
                    <img className="cursor-pointer hover:scale-[1.2] transform transition-transform duration-[400ms]" src={`../../Ekart/Images/${title}.png`} alt="" />
                </Link>
                <div
                    onClick={() => handleLike()}
            ...
            <div className="flex">
                {[...Array(5)].map((e, i) => (
                    i < rating
                        ? <StarIconSolid key={i} className="xsm-mx:h-3 xsm-mx:w-3 h-4 text-green-500 w-4" />
                        : <StarIcon key={i} className="xsm-mx:h-3 xsm-mx:w-3 h-4 text-green-500 w-4" />
                ))}
            ...
            <div className="flex justify-between">
                {!added && !goToCart && (
                    <button
                        onClick={() => {
                            dispatch(addItem({ id, title, price, desc, rating, "quantity": 1 }));
                            handleAdd();
                        }}
                ...
                {added && (
                    <button
                        className="flex border items-center border-green-500 text-green-500 text-sm focus:outline-none hover:bg-green-500 hover:text-white py-1.5 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px] md-mx:text-xs md-mx:py-1 md-mx:px-2"
                    >
                        <CheckIcon className="font-extrabold sm-mx:w-4 sm-mx:h-4 w-5 h-5" /> Added
                    </button>
                )}
                {goToCart && (
                    <Link to={`/cart`}>
                        <button
                            className="border border-blue-500 text-sm focus:outline-none bg-blue-500 text-white py-1.5 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px] md-mx:text-xs md-mx:py-1 md-mx:px-2"
                        >
                            Go to Cart
                        </button>
                    </Link>
