...
const SimilarProd = ({ id, title, price, rating }) => {
    return (
        <Link to={`/product/${id}`}>
            <div className="bg-gray-100 cursor-pointer hover:bg-gray-200 flex items-center p-1 py-2 sm-mx:min-h-28 rounded-xl">
                <div className="flex gap-1 sm-mx:justify-between items-center">
                    ...
                    <div className="flex w-36 sm-mx:w-[90px] flex-col gap-1">
                        ...
                        <div className="flex">{
                            [...Array(5)].map((e, i) => {
                                if (i < rating) {
                                    return <StarIconSolid key={i} className="xsm-mx:h-2 xsm-mx:w-2 h-3 text-green-500 w-3" />;
                                } else {
                                    return <StarIcon key={i} className="xsm-mx:h-2 xsm-mx:w-2 h-3 text-green-500 w-3" />;
                                }
                            })
                        }
                        </div>
