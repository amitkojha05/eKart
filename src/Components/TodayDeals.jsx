...
const TodayDeals = () => {
    ...
    }, [btn, category]); // Include category in dependency array
        ...
        <div id="deals" className="scroll-smooth p-5 px-3 w-full">
            ...
            <div className="flex gap-2 my-3 flex-wrap">
                {category.map((e, i) => (
                    <button
                        key={i}
                        className={`border focus:outline-none hover:border-blue-500 border-gray-500 text-gray-800 text-sm hover:bg-blue-500 hover:text-white ${btn === i ? "bg-blue-500 text-white border-blue-500" : ""}  py-1.5 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px] md-mx:text-xs md-mx:py-1 md-mx:px-2`}
                        onClick={() => setBtn(i)}
                    >
                        {e}
                    </button>
