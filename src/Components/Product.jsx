    ...
    useEffect(() => {
        getItem(id).then((res) => {
            ...
            getCategoryWiseItems(res.category).then((res) => {
                ...
                setFlag(false);
                res.forEach((e, i) => {
                    if (i < 6 && e.id === parseInt(id)) setFlag(true);
                });
            });
            ...
            <div className="flex max-w-[272px] flex-col gap-3">
                ...
                <div className="flex gap-4 items-center">
                    <div className="flex gap-4 rounded-xl text-sm font-semibold items-center px-3 py-1.5 bg-gray-100 w-fit">
                        <MinusIcon className="w-4 h-4 font-extrabold cursor-pointer text-red-500" onClick={() => setCount(Math.max(count - 1, 1))} />
                        <span>{count}</span>
                        <PlusIcon className="cursor-pointer w-4 h-4 text-green-600" onClick={() => setCount(Math.min(count + 1, 10))} />
                    </div>
                ...
                <div className="flex gap-2">
                    ...
                    {!added && !goToCart && (
                        <button onClick={() => { dispatch(addItem({ ...product, "quantity": count })); handleAdd() }} className="border focus:outline-none font-semibold border-blue-500 text-blue-500 text-sm hover:bg-blue-500 hover:text-white py-2 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px]">Add to Cart</button>
                    )}
                    {added && (
                        <button className="flex border border-green-500 text-sm focus:outline-none bg-green-500 text-white font-semibold py-2 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px]">
                            <CheckIcon className="font-extrabold w-5 h-5" /> Added
                        </button>
                    )}
                    {goToCart && (
                        <Link to={`/cart`}>
                            <button className="border focus:outline-none font-semibold border-blue-500 text-white bg-blue-500 text-sm hover:bg-blue-800 py-2 px-4 rounded-full transition duration-300 ease-in-out xsm-mx:text-[10px]/[12px]">Go to Cart</button>
                        </Link>
                    )}
                </div>
            </div>
